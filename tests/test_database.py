...
class TestSchema:
    def test_connect_creates_tables(self, db):
        ...
        assert "incompatibilities" in tables
        assert "issues" in tables
        assert "load_order_rules" in tables
...
class TestRequirements:
    ...
    def test_upsert_requirements_replaces_old(self, db):
        db.upsert_mod({"mod_id": 501, "name": "ModB", "last_scraped": "2024-01-01"})
...
class TestIssues:
    def test_upsert_and_get_issues(self, db):
        db.upsert_mod({"mod_id": 600, "name": "IssuesMod", "last_scraped": "2024-01-01"})
        issues = [
            {
                "title": "Bug report",
                "body": "Game crashes",
        ...
        db.upsert_issues(600, issues)
        stored = db.get_issues(600)
        assert len(stored) == 1
        assert stored[0]["title"] == "Bug report"


class TestLoadOrderRules:
    def test_upsert_and_get_rules(self, db):
        db.upsert_mod({"mod_id": 700, "name": "RulesMod", "last_scraped": "2024-01-01"})
        rules = [
            {"rule_type": "AFTER", "target_mod_name": "Unofficial Skyrim Patch", "notes": ""},
        ]
        db.upsert_load_order_rules(700, rules)
        stored = db.get_load_order_rules(700)
        assert len(stored) == 1
        assert stored[0]["rule_type"] == "AFTER"
        assert stored[0]["target_mod_name"] == "Unofficial Skyrim Patch"
...
class TestContextManager:
    def test_context_manager(self, tmp_path):
        ...
        with DatabaseManager(db_path=path) as db:
            db.upsert_mod({"mod_id": 1, "name": "CTX Mod", "last_scraped": "2024-01-01"})
