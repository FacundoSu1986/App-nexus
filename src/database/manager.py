...
class DatabaseManager:
    ...
    def upsert_ai_analysis(self, nexus_id: str, analysis: dict, analyzed_by: str) -> None:
        """Insert or replace an AI analysis result for a mod."""
        try:
            self.conn.execute(
                """
                INSERT INTO ai_mod_analysis
                    (nexus_id, requirements, patches, known_issues, analyzed_by, last_analyzed)
                VALUES
                    (:nexus_id, :requirements, :patches, :known_issues, :analyzed_by, :last_analyzed)
            ...
            logger.error("SQL error upserting AI analysis for mod %s: %s", nexus_id, exc)
            raise

    def get_ai_analysis(self, nexus_id: str) -> Optional[dict]:
        """Fetch the stored AI analysis for a mod by nexus_id.  Returns None if absent."""
        row = self.conn.execute(
