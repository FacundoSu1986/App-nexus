...
class MainWindow(tk.Tk):
    ...
    def _sync_mods(self) -> None:
        ...
        Fetches metadata via the API and requirements via the official endpoint.
        """
        ...
        for idx, mod in enumerate(mods, start=1):
            ...
            local_results = self._db.search_mods_by_name(mod.name)
            exact_match = any(
                r["name"].lower() == mod.name.lower() for r in local_results
            )
            if exact_match:
                self.after(
                    0, self._set_status, f"Skipping '{mod.name}' (already cached)."
                )
                continue
            ...
            try:
                ...
                requirements = self._api.get_mod_requirements(mod_id)  # type: ignore[union-attr]
                if requirements:
