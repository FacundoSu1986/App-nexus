...
│                │  Incompatibilities / Load Order / Issues  │
├────────────────┴───────────────────────────────────────────┤
│  Report panel (missing mods, patches, conflicts)           │
└────────────────────────────────────────────────────────────┘
"""
...
from src.nexus.scraper import NexusScraper
...
class MainWindow(tk.Tk):
    ...
    def _sync_mods(self) -> None:
        """
        Iterate over the enabled mods and try to find their Nexus page.
        Fetches metadata via the API and scrapes requirements/issues.
        """
        scraper = NexusScraper()
        mods = self._profile.enabled_mods  # type: ignore[union-attr]
        total = len(mods)
        for idx, mod in enumerate(mods, start=1):
            self._set_status(f"Syncing mod {idx}/{total}: {mod.name}…")
            try:
                results = self._api.search_mods(mod.name)  # type: ignore[union-attr]
                if not results:
                    continue
                best = results[0]
                mod_id = best["mod_id"]
                full_data = self._api.get_mod(mod_id)  # type: ignore[union-attr]
                self._db.upsert_mod(full_data)

                requirements = scraper.get_requirements(mod_id)
                if requirements:
                    self._db.upsert_requirements(mod_id, requirements)

                issues = scraper.get_issues(mod_id, max_pages=1)
                if issues:
                    self._db.upsert_issues(mod_id, issues)
                ...
                self.after(
                    ...
                    messagebox.showwarning,
                    "Rate Limit",
                    "Nexus Mods API rate limit reached. "
                    "Free accounts allow 100 requests/day.",
                )
                break
            except NexusAPIError as exc:
                self._set_status(f"API error for '{mod.name}': {exc}")
            except Exception as exc:
                self._set_status(f"Error syncing '{mod.name}': {exc}")

        self.after(0, self._set_status, "Sync complete.")
        self.after(0, self._populate_mod_list)
    ...
    def _display_report(self, report: dict) -> None:
        ...
        lines = [
            ...
            f"  Incompatible  : {stats['incompatible_count']}",
            f"  Load-order ⚠  : {stats['violations_count']}",
            "╚══════════════════════════════════════════════════════════╝",
            "",
        ]
        ...
        if report["incompatibilities"]:
            ...
            for inc in report["incompatibilities"]:
                lines.append(
                    f"  ⚠ '{inc['mod_name']}' conflicts with '{inc['incompatible_name']}'"
                )
                if inc.get("reason"):
                    lines.append(f"    Reason: {inc['reason']}")
            lines.append("")

        if report["load_order_violations"]:
            lines.append("── LOAD ORDER VIOLATIONS ──")
            for v in report["load_order_violations"]:
                lines.append(
                    f"  '{v['mod_name']}' should load {v['rule_type']} "
                    f"'{v['target_name']}' "
                    f"(current: #{v['current_index']}, target: #{v['target_index']})"
                )
            lines.append("")
